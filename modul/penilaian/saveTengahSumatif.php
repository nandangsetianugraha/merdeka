...
if(is_numeric($nilai)){
    if($nilai>100){}else{
        $hasil=$connect->query("select * from sts where id_pd='$idp' AND kelas='$ab' AND smt='$smt' AND tapel='$tapel' AND mapel='$mpid'")->fetch_assoc();
        $ada = $connect->query("select * from sts where id_pd='$idp' AND kelas='$ab' AND smt='$smt' AND tapel='$tapel' AND mapel='$mpid'")->num_rows;
        if ($ada>0){
			$utt=$connect->query("select * from sts where id_pd='$idp' AND kelas='$ab' AND smt='$smt' AND tapel='$tapel' AND mapel='$mpid'")->fetch_assoc();
        	$idn=$utt['idNH'];
        	...
        	$sql = "INSERT INTO sts(id_pd, kelas, smt, tapel, mapel, nilai) VALUES('$idp','$ab','$smt','$tapel','$mpid','$nilai')";
