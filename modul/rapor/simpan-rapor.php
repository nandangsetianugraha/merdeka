...
if($_POST) {	
	...
	if(empty($smt) || empty($pdid)){
		...
		if($ceksts>0 or $ceksas>0){
			$kelebihan=$connect->query("SELECT * FROM formatif WHERE id_pd='$pdid' AND kelas='$kelas' AND smt='$smt' AND tapel='$tapel' AND mapel='$mp' order by nilai desc")->fetch_assoc();
			$kelemahan=$connect->query("SELECT * FROM formatif WHERE id_pd='$pdid' AND kelas='$kelas' AND smt='$smt' AND tapel='$tapel' AND mapel='$mp' order by nilai asc")->fetch_assoc();
			$lm1=$kelebihan['lm'];
			$tp1=$kelebihan['tp'];
			$lm2=$kelemahan['lm'];
			$tp2=$kelemahan['tp'];
			...
			$rataFor=$connect->query("SELECT AVG(nilai) as rataformatif FROM formatif WHERE id_pd='$pdid' AND kelas='$kelas' AND smt='$smt' AND tapel='$tapel' AND mapel='$mp'")->fetch_assoc();
